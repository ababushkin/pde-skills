...
These are the ten beliefs that inform every engineering decision we make. If a decision conflicts with a principle, the principle wins by default — and the burden of justification sits with the person arguing to deviate.
1. Shipped is not done; observed is done
Deployed code is necessary but not sufficient. A change is only "done" when it is running in production, instrumented, and the thing it was meant to move has been observed moving — or decisively not moving. This is the engineering translation of the roadmap's "outcome achieved, not feature shipped." A decade of DORA research makes the evidence overwhelming: high-performing teams deploy small changes often and see the effect quickly; low-performing teams ship occasionally, discover problems weeks later, and spend their lives in firefighting. The 2024 State of DevOps report reinforces this with a timely wrinkle — AI-assisted coding increases individual output, but teams that let batch size grow alongside that output see delivery throughput decrease. Deployment without observation is theatre.
...
Shipped is not done; observed is done
Design starts with the problem, not the stack
Architecture is the set of expensive-to-change decisions — minimise it
Name your assumptions; test the risky ones before committing
Code is a liability; the best change is often the one you don't make
Operability, observability, and security are functional requirements
Design for change; the system will outlive its requirements
Small batches beat big batches — every time
Conway's Law is a law, not a suggestion
Prefer boring technology; innovation tokens are scarce
