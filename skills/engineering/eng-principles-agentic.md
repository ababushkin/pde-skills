...
**Small batches matter MORE.** The 2024 DORA report's load-bearing finding is specific: AI-assisted coding raises individual output but degrades stability when batch size grows — a ~7% drop in delivery throughput observed across teams that let batch size grow alongside agent productivity. The discipline of universal Principle 8 is the brake on this. Without it, the productivity gains evaporate. The same DORA work also notes that AI-assisted teams that hold batch size constant see the gains compound rather than offset.
